...
export class CartDetailController {
            ...
            CartEntity.findOne({ where: { productID: cart.productID } }).then(async result => {
                if (result) {
                    ...
                    ProductEntity.findOne({ where: { productID: cart.productID } }).then(async result2 => {
                        if (result2) {
                            try {
                                CartEntity.create(cart).then(result3 => {
    ...
    ///<===== plusAmount Product ======>
    public static plusAmount(eq: Request, res: Response) {
    ...
    public static updateCart(req: Request, res: Response) {
        ...
        if (Object.entries(cart).length === 0) {
            ...
            CartEntity.findOne({ where: { cartID: cart.cartID, userID: cart.userID } }).then(async result => {
                if (result) {
                    try {
                        Service.copyObject(cart, result);
                        await result.save();
    ...
    public static getCart(req: Request, res: Response) {
        ...
        if (cart.userID) {
            ...
            try {
                CartEntity.findAll({ where: { userID: cart.userID } }).then(result => {
                    if (result) {
